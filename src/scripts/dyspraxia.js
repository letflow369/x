(() => {
  "use strict";
  document.documentElement.classList.add("dyspraxia-js");

  const initToc = () => {
    const links = Array.from(document.querySelectorAll(".article-toc a[href^='#']"));
    if (!("IntersectionObserver" in window) || !links.length) return;
    const byId = new Map(links.map((link) => [link.getAttribute("href").slice(1), link]));
    const sections = Array.from(document.querySelectorAll(".article-content .article-section[id], .dyspraxia-summary[id]"));
