(() => {
  ...
  const sideLinks = [...document.querySelectorAll('.pn-sidenav a[href^="#"]')];
  ...
  const filterGroup = document.querySelector('.pn-source-filters');
  const references = [...document.querySelectorAll('.pn-reference-list details[data-source-type]')];
  ...
  document.addEventListener('click', (event) => {
    const link = event.target.closest('a.pn-ref[href^="#ref-"]');
    if (!link) return;
