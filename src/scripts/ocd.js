(() => {
  ...
  const initializeTabs = () => {
    document.querySelectorAll("[data-ocd-tabs]").forEach((root) => {
      if (root.dataset.ready === "true") return;
      root.dataset.ready = "true";

      const tabs = Array.from(root.querySelectorAll("[data-ocd-tab]"));
      const panels = Array.from(root.querySelectorAll("[data-ocd-panel]"));

      const activate = (tab, moveFocus = false) => {
        const targetId = tab.dataset.ocdTab;
  ...
  const initializeAccordion = () => {
    document.querySelectorAll("[data-ocd-accordion]").forEach((group) => {
