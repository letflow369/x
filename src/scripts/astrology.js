(() => {
  ...
  const initializeTimeline = () => {
    const timeline = document.querySelector("[data-astrology-timeline]");
    const controls = document.querySelector("[data-astrology-timeline-controls]");
    ...
    const select = (index, shouldFocus = false) => {
      ...
      if (shouldFocus) buttons[currentIndex]?.focus();
    };
  ...
  const initializeAccordion = () => {
    document.querySelectorAll("[data-astrology-accordion]").forEach((group) => {
    ...
    document.querySelectorAll("[data-copy-reference]").forEach((button) => {
      button.addEventListener("click", async () => {
        try {
          ...
          if (status) {
            status.textContent = "Referência copiada para a área de transferência.";
          }
        } catch {
          if (status) {
            status.textContent = "Não foi possível copiar automaticamente. Selecione o texto da referência.";
          }
        }
      });
    });
